...
;layui.define(["layer", "laytpl", "upload"], function(i) {
    ...
    var u = function(i) {
        ...
        , v = '<li class="layim-{{ d.data.type }}{{ d.data.id }} layim-chatlist-{{ d.data.type }}{{ d.data.id }} layim-this" layim-event="tabChat"><img src="{{ d.data.avatar }}"><span>{{ d.data.name||"佚名" }}</span>{{# if(!d.base.brief){ }}<i class="layui-icon" layim-event="closeChat">&#x1007;</i>{{# } }}</li>'
        , g = function(i) {
        return i < 10 ? "0" + (0 | i) : i
    };
    ...
    var x, b, w, k, C, S = function(i, a, n) {
        ...
        return mine = a.mine || {},
            ...
            create = function(a) {
                ...
                j = e.extend(j, s),
                    L(n(y).render(s)),
                (l.close || i.min) && T(),
                    layui.each(r.ready, function(i, a) {
                        a && a(s)
                    })
            }
            ,
            j = e.extend(j, obj),
            i.brief ? layui.each(r.ready, function(i, a) {
                a && a(obj)
        ...
        if (a[0]) {
            ...
            1 !== s.find("li").length || o[0] || (d || w.css("width", 800),
                s.css({
                    height: w.height()
                }).show(),
                c.css("margin-left", "200px")),
            o[0] || (s.append(n(v).render(l)),
                c.append(n(f).render(l)),
                I(i),
                E()),
                O(s.find(".layim-chatlist-" + i.type + i.id)),
            o[0] || U(),
        ...
        var u = C = t.open({
            ...
            success: function(a) {
                ...
                "function" == typeof i.success && i.success(a),
                    ...
                    U(),
                    q(),
                    layui.each(r.chatChange, function(i, a) {
                        a && a(_())
                    }),
                    a.on("dblclick", ".layui-layim-photos", function() {
                        var i = this.src;
                        t.close(z.photosIndex),
                            t.photos({
                                photos: {
                                    data: [{
                                        alt: "大图模式",
                ...
                success: function(a, l) {
                    ...
                    n.minRight && t.style(l, {
                        ...
                        a.find(".layui-layer-content span").on("click", function() {
                            t.close(l),
                                i ? layui.each(j.chat, function(i, a) {
                                    z(a)
                                }) : w.show(),
                            i && (j.chat = [],
                                Z())
        ...
        if (a) {
            ...
            return 1 === u ? t.close(C) : (i.remove(),
                d.remove(),
            2 === u && (w.find(".layim-chat-list").hide(),
            c || w.css("width", "600px"),
                w.find(".layim-chat-box").css("margin-left", 0)),
                !1)
        }
    ...
    }, _ = c.prototype.thisChat = function() {
        if (w) {
        ...
        if (x) {
            ...
            if (i.historyTime = (new Date).getTime(),
                t[i.type + i.id] = i,
                a.history = t,
                layui.data("layim", {
                    key: j.mine.id,
        ...
        if (i.content = a.textarea.val(),
        "" !== i.content.replace(/\s/g, "")) {
            if (i.content.length > l)
                return t.msg("内容最长不能超过" + l + "个字符");
            ...
            B(o),
                layui.each(r.sendMessage, function(i, a) {
                    a && a(s)
                })
        }
        Z(),
        ...
        var a = e(".layim-chatlist-" + i.type + i.id)
            , t = {}
            , l = a.index();
        if (i.timestamp = i.timestamp || (new Date).getTime(),
        i.fromid == j.mine.id && (i.mine = !0),
        i.system || B(i),
            G = JSON.parse(JSON.stringify(i)),
        j.base.voice && F(),
        !w && i.content || l === -1) {
            if (j.message[i.type + i.id])
                j.message[i.type + i.id].push(i);
            else if (j.message[i.type + i.id] = [i],
            "friend" === i.type) {
                var s;
            ...
            if ("group" === i.type && layui.each(j.group, function(a, e) {
                if (e.id == i.id)
                    return t.avatar = e.avatar,
                        !0
            }),
                !i.system)
                return j.base.notice && J({
        ...
        if (w) {
            var d = _();
            d.data.type + d.data.id !== i.type + i.id && (a.addClass("layui-anim layer-anim-06"),
                setTimeout(function() {
                    a.removeClass("layui-anim layer-anim-06")
                }, 300));
            var r = w.find(".layim-chat").eq(l)
                , c = r.find(".layim-chat-main ul");
            i.system ? l !== -1 && c.append('<li class="layim-chat-system"><span>' + i.content + "</span></li>") : "" !== i.content.replace(/\s/g, "") && c.append(n(h).render(i)),
                Z()
        }
    }, Y = "layui-anim-loop layer-anim-05", V = function(i) {
        ...
        var a, e = {}, l = x.find(".layim-list-" + i.type);
        if (j[i.type])
            if ("friend" === i.type)
                layui.each(j.friend, function(n, l) {
                    if (i.groupid == l.id)
                        return layui.each(j.friend[n].list, function(e, t) {
                            if (t.id == i.id)
                                return a = !0
                        }),
                            a ? t.msg("好友 [" + (i.username || "") + "] 已经存在列表中", {
                                anim: 6
                            }) : (j.friend[n].list = j.friend[n].list || [],
                                e[j.friend[n].list.length] = i,
                                i.groupIndex = n,
                                j.friend[n].list.push(i),
                                !0)
                });
            else if ("group" === i.type) {
                if (layui.each(j.group, function(e, t) {
                    ...
                    a)
                    return t.msg("您已是 [" + (i.groupname || "") + "] 的群成员", {
                        anim: 6
                    });
                e[j.group.length] = i,
                    j.group.push(i)
        ...
        var i = _()
            , a = i.textarea;
        a.focus(),
            a.off("keydown").on("keydown", function(i) {
                var e = layui.data("layim")[j.mine.id] || {}
                    , t = i.keyCode;
                if ("Ctrl+Enter" === e.sendHotKey)
                    return void (i.ctrlKey && 13 === t && D());
                if (13 === t) {
                    if (i.ctrlKey)
                        return a.val(a.val() + "\n");
                    if (i.shiftKey)
        ...
        return layui.each(i, function(i, e) {
            ...
            a
    }(), ii = layui.stope, ai = function(i, a) {
        var e, t = i.value;
        i.focus(),
            document.selection ? (e = document.selection.createRange(),
                document.selection.empty(),
                e.text = a) : (e = [t.substring(0, i.selectionStart), a, t.substr(i.selectionEnd)],
                i.focus(),
                i.value = e.join(""))
    }, ei = "layui-anim-upbit", ti = {
        status: function(i, a) {
            var t = function() {
            ...
            if ("show" === n)
                ...
                i.addClass(o).siblings().removeClass(o),
                    ...
                    layui.each(r.online, function(i, a) {
                        a && a(n)
                    })
            }
        },
        ...
        tab: function(i) {
            var a, e = ".layim-tab-content", t = x.find(".layui-layim-tab>li");
            "number" == typeof i ? (a = i,
                i = t.eq(a)) : a = i.index(),
                a > 2 ? t.removeClass(o) : (ti.tab.index = a,
                    i.addClass(o).siblings().removeClass(o)),
        ...
        search: function(i) {
            var a = x.find(".layui-layim-search")
                ...
                if ("" === a)
                    ti.tab(0 | ti.tab.index);
                else {
                    for (var n = [], l = j.friend || [], s = j.group || [], o = "", d = 0; d < l.length; d++)
                        for (var r = 0; r < (l[d].list || []).length; r++)
                            l[d].list[r].username.indexOf(a) !== -1 && (l[d].list[r].type = "friend",
                                l[d].list[r].index = d,
                                l[d].list[r].list = r,
                                n.push(l[d].list[r]));
                    for (var c = 0; c < s.length; c++)
                        s[c].groupname.indexOf(a) !== -1 && (s[c].type = "group",
                            s[c].index = c,
                            s[c].list = c,
                            n.push(s[c]));
                    if (n.length > 0)
                        for (var u = 0; u < n.length; u++)
                            o += '<li layim-event="chat" data-type="' + n[u].type + '" data-index="' + n[u].index + '" data-list="' + n[u].list + '"><img src="' + n[u].avatar + '"><span>' + (n[u].username || n[u].groupname || "佚名") + "</span><p>" + (n[u].remark || n[u].sign || "") + "</p></li>";
                    else
                        o = '<li class="layim-null">无搜索结果</li>';
                    e.html(o),
                        ti.tab(3)
                }
            };
            !j.base.isfriend && j.base.isgroup ? ti.tab.index = 1 : j.base.isfriend || j.base.isgroup || (ti.tab.index = 2),
                a.show(),
                t.focus(),
        ...
        msgbox: function() {
            ...
            return t.close(ti.msgbox.index),
                i.find("span").removeClass(Y).html(""),
                ti.msgbox.index = t.open({
                    type: 2,
                    title: "消息盒子",
                    shade: !1,
                    maxmin: !0,
        ...
        groupMembers: function(i, a) {
            var n = i.find(".layui-icon")
                ...
                n.html("&#xe61a;"),
                    i.data("down", null),
                    t.close(ti.groupMembers.index)
            }
                , s = function(i) {
                ii(i)
            };
            i.data("down") ? l() : (n.html("&#xe619;"),
                i.data("down", !0),
                ti.groupMembers.index = t.tips('<ul class="layim-members-list"></ul>', i, {
                    tips: 3,
                    time: 0,
                    anim: 5,
                    fixed: !0,
                ...
                e(document).off("mousedown", l).on("mousedown", l),
                e(window).off("resize", l).on("resize", l),
                i.off("mousedown", s).on("mousedown", s))
        },
        send: function() {
            D()
        },
        setSend: function(i, a) {
            ...
            if ("show" === n)
                ii(a),
                    t.show().addClass(ei),
                    e(document).off("click", ti.setSendHide).on("click", ti.setSendHide);
            else {
                i.addClass(o).siblings().removeClass(o);
                ...
                ti.face.index = t.tips(n, i, {
                    ...
                    success: function(i) {
                        i.find(".layim-face-list>li").on("mousedown", function(i) {
                            ...
                            ai(l.textarea[0], "face" + this.title + " "),
                                t.close(ti.face.index);
                        })
                    }
                }),
                e(document).off("mousedown", ti.faceHide).on("mousedown", ti.faceHide),
                e(window).off("resize", ti.faceHide).on("resize", ti.faceHide),
                ii(a)
        },
        faceHide: function() {
            t.close(ti.face.index)
            ...
            layui.upload.render({
                ...
                accept: a,
                    ...
                    0 == i.code ? (i.data = i.data || {},
                        "images" === a ? ai(n.textarea[0], "img[" + (i.data.src || "") + "]") : "file" === a && ai(n.textarea[0], "file(" + (i.data.src || "") + ")[" + (i.data.name || "下载文件") + "]"),
                        D()) : t.msg(i.msg || "上传失败")
                }
            })
        },
        ...
        playAudio: function(i) {
            ...
            return i.data("error") ? t.msg("播放音频源异常") : e.play ? void (i.attr("status") ? n() : (a || (e.src = i.data("src")),
                    e.play(),
                    i.attr("status", "pause"),
                    i.data("audio", e),
                    i.find("i").html("&#xe651;"),
        ...
        playVideo: function(i) {
            ...
            return e.play ? (t.close(ti.playVideo.index),
                void (ti.playVideo.index = t.open({
                    type: 1,
                    title: "播放视频",
                    area: ["460px", "300px"],
                    maxmin: !0,
        ...
        chatLog: function(i) {
            var a = _();
            return j.base.chatLog ? (t.close(ti.chatLog.index),
                ti.chatLog.index = t.open({
                    type: 2,
                    maxmin: !0,
    ...
    i("layim", new c)
}).addcss("modules/layim/layim.css?v=3.8.0", "skinlayimcss");
